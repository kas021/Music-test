...
(function () {
  ...
  function callPlayer(trackId, client, userAgent) {
    ...
    return post(
      YTM_BASE + '/youtubei/v1/player?prettyPrint=false',
      { context: { client: context }, videoId: trackId,  },
      ...
      if (status && status !== 'OK') {
        ...
        var blocked = new Error('Playback unavailable for this source.');
        blocked.playbackBlocked = true;
        throw blocked;
      }
      return data || {};
    });
  ...
  function getVisitorData() {
    ...
    return post(
      YTM_BASE + '/youtubei/v1/visitor_id',
  ...
  function resolvePlayback(trackId, quality, extra) {
    return callPlayer(trackId, IOS_CONTEXT, IOS_USER_AGENT).then(function (data) {
      ...
      if (err && err.playbackBlocked) throw err;
      // Retry transport/format failures only, never explicit access restrictions.
